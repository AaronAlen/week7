...
export const PendingApprovals: React.FC = () => {
  ...
  // Modal for testing AI refund agent
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [restockRes, refundRes] = await Promise.all([
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Monitor, audit, and authorize high-risk autonomous AI decisions across Procurement and Customer Support.
          </p>
        </div>
          ...
          {approvals.length === 0 ? (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-10 text-center space-y-3">
              ...
              <p className="text-xs text-slate-400 max-w-md mx-auto">
                No high-value purchase orders currently awaiting authorization. Restock requests &le; $1,000 are autonomously dispatched by the LangGraph AI Procurement Agent.
              </p>
            </div>
          ) : (
            approvals.map((item) => {
                ...
                <div key={item.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-slate-800 gap-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xs text-slate-400 font-mono">Thread: {item.threadId.substring(0, 18)}...</span>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                    ...
                    <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800 space-y-1.5">
                      ...
                      <p className="text-amber-600 dark:text-amber-400 font-medium">⚠️ Exceeds $1,000 autonomous spending limit</p>
                  ...
                  <div className="bg-blue-950/20 border border-blue-500/20 rounded-xl p-3 text-xs space-y-1">
                    <span className="font-bold text-blue-700 dark:text-blue-300 flex items-center space-x-1.5">
                      ...
                      <span>Autonomous AI Restock Reasoning</span>
                    </span>
                    ...
                    <div className="bg-indigo-950/30 border border-indigo-500/30 rounded-xl p-3 space-y-1.5">
                      ...
                      {ref.customerEmailDraft && (
                        <div className="pt-2 border-t border-indigo-500/20 text-[11px] text-slate-400">
                          <p className="font-semibold text-indigo-700 dark:text-indigo-200 mb-1 flex items-center space-x-1">
